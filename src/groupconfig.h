...
public:
    ...
    // 0 - group name or property with specified key is not present
    // 1 - otherwise
    virtual Property getProperty(const std::string &name, const std::string &key, int *set = nullptr) const = 0;
    ...
    // 1 - otherwise
    // if you already read the configuration, use 'get' for the value
    virtual Property readProperty(const std::string &name, const std::string &key, int *set = nullptr) const = 0;
};
